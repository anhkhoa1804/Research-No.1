#!/usr/bin/env python
"""SECONDARY DIAGNOSTIC ONLY -- verbatim copy of tools/appearance_probe.py
plus report-only additions. It fits NOTHING differently.

Why a copy rather than an edit: the L/14-336 run is pre-registered
(docs/APPEARANCE_PROBE_L14_PREREGISTRATION.md) and the verdict-bearing script
must stay byte-identical to the version registered at 91b24c7. This variant
exists solely to compute the pre-registered SECONDARY denominator of that
document's section 5 -- `captured_decidable`, the share of headroom captured
when the class average is restricted to the 27 DECIDABLE predicates -- which
the original script does not emit.

Everything added below is arithmetic over counters the original already
computes. No RNG is drawn, no tensor is re-fitted, and nothing is reordered,
so this file consumes the global RNG stream in exactly the same order as the
original and MUST reproduce its primary numbers bit-for-bit. That equality is
the check that makes the secondary number trustworthy: if the primary numbers
differ at all, this file is wrong and its output must be discarded.

The 5% decision threshold applies to captured_total ONLY. Nothing here can
change the verdict.
...
# Single source of truth for the generic/decidable split, so this diagnostic
# cannot drift from runs/p3e_headroom_train_derived/ which used the same set.
from headroom_analysis import GENERIC_PREDICATES  # noqa: E402
...
def score(cache: Path, out_path: str) -> Dict[str, Any]:
    ...
    Ro, mo, bko, pho, pgo = mr_of(torch.where(in_va, Yva, Pva.argmax(-1)), Yva)
    ...
    # -----------------------------------------------------------------
    # SECONDARY DENOMINATOR (pre-registration section 5), report-only.
    # Same measurement, class average restricted to DECIDABLE predicates.
    # 64.2 % of the prior's errors are generic->generic (`on`/`in`/`of`),
    # which are annotation-style choices no encoder can resolve, so the
    # primary denominator is largely unreachable by construction. This
    # number is more informative and CANNOT change the verdict.
    # -----------------------------------------------------------------
    dec_idx = [k for k in range(P) if PV[k] not in GENERIC_PREDICATES]
    res["decidable_predicates"] = sorted(PV[k] for k in dec_idx)
    res["n_decidable_predicates"] = len(dec_idx)

    def _dec_mR(ph_c, pg_c):
        ks = [k for k in pg_c if k in set(dec_idx) and pg_c[k] > 0]
        if not ks:
            return None, 0, 0
        n_inst = sum(pg_c[k] for k in ks)
        return sum(ph_c[k] / pg_c[k] for k in ks) / len(ks), len(ks), n_inst

    dec_prior, n_dec_cls, n_dec_inst = _dec_mR(ph0, pg0)
    dec_oracle, _, _ = _dec_mR(pho, pgo)
    dec_add = _dec_mR(ph_best, pg_best)[0] if ph_best is not None else None

    res["decidable_classes_present"] = n_dec_cls
    res["decidable_val_instances"] = n_dec_inst
    res["decidable_prior_mR"] = dec_prior
    res["decidable_oracle_mR"] = dec_oracle
    res["decidable_additive_mR"] = dec_add
    if None not in (dec_prior, dec_oracle, dec_add):
        cap_dec = (dec_add - dec_prior) / max(1e-9, (dec_oracle - dec_prior)) * 100
        res["captured_decidable_pct"] = cap_dec
        res["captured_total_pct"] = res["best_additive_headroom_pct"]
        print("\n" + "=" * 86)
        print("SECONDARY DENOMINATOR -- decidable predicates only (report-only, "
              "cannot change the verdict)")
        print("=" * 86)
        print(f"  decidable classes present : {n_dec_cls} of {len(dec_idx)}  "
              f"({n_dec_inst:,} validation instances)")
        print(f"  P0 prior      mR (decidable): {dec_prior * 100:6.2f}%")
        print(f"  ORACLE@5      mR (decidable): {dec_oracle * 100:6.2f}%")
        print(f"  additive      mR (decidable): {dec_add * 100:6.2f}%   "
              f"(lambda={best_lam}, selected on held-out train)")
        print(f"  captured_decidable          : {cap_dec:.1f}%   "
              f"vs captured_total {res['best_additive_headroom_pct']:.1f}%")
