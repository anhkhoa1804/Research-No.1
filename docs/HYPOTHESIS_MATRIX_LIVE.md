...
| **H6** | **Representation bottleneck** | **MR** every probe on 768-d `rel_feat` sits **0.023–0.036 below** a linear model on 19 box numbers (`p37`, `p55`). **MR** `R9` rel_feat+geometry (0.5735) < geometry alone (0.5961) — adding the representation **destroys** information. **MR** collapse R² only 18.25%, so it is not an object-identity re-encoding. **STRENGTHENED BY ELIMINATION**: the objective route is now refuted on `rel_feat` itself (`p55`) and the supervision route is refuted (`p56`). **MR `p60` — the confound test the claim most needed.** Under ONE estimator and ONE regime applied identically, geometry 0.5976 vs `rel_feat` 0.5732, `delta_est = +0.0244`, CIs disjoint, all gates pass: the deficit is **not** a fitting artifact. **MR `p60`** `C_fusion` 0.5922 < `B_geometry` 0.5976 — the additive-fusion route is closed on a *matched* measurement. **MR `p59`** the whole verdict replicates on the held-out TEST split (`P*−C = −0.0116`, `P*−G = −0.0320`). **MR `p65` (preregistered): the minimal `dx_rel`/`dy_rel` fix is NEUTRAL** (+0.0050, threshold +0.02) **and fusing the CLEANED (group-centred) `rel_feat` with geometry is actively HARMFUL** (−0.0320, past the −0.02 threshold) — worse than raw fusion, not merely redundant. **MR `p66` (preregistered): `dy_rel` IS nonlinearly present in `rel_feat`** (MLP R²=0.292 vs linear 0.223, CI-separated from the estimator's own empirical permutation null; `dx_rel` weakly so, R²=0.073) — the deficit is partly a *readout*, not *representation*, limitation for `dy_rel` specifically. **MR `p67` (preregistered): but an auxiliary regression loss forcing the readout to retain that same signal is also NEUTRAL** (delta_aux = −0.0001, flat across a 4x lambda sweep) — supervising the readout directly toward the one thing `p66` showed it under-uses still does not move discrimination. Four additive-fusion attempts across two estimator families (`p37`, `p60`, `p65`) plus the minimal literal-feature-injection fix and the auxiliary-training-signal fix (`p65`, `p67`) have now all failed to beat geometry alone or to move WPRD at all | **MR** `p55` withdraws `p37`'s R5: group-centring gains under a linear probe (0.5807) but **collapses under an MLP** (0.5173), so the "active distractor" reading is a linear-probe property, not a representation property. **MR `p65`** confirms this from the fusion side too. **MR `p67`** confirms it a third way: even direct auxiliary supervision toward the specific quantity `p66` found recoverable does not transfer into discrimination | **SUPPORTED — the only surviving structural account, HELD-OUT REPLICATED (`p59`), and now exhausted for every additive/frozen-readout AND readout-training-signal fix tried (`p65`, `p67`)** | joint-training pilot (untested, reduced prior after `p67`) or cross-model |
...
`p55` (objective), `p65` (the minimal `dx_rel`/`dy_rel` fix, NEUTRAL;
cleaned-fusion, HARMFUL), and now `p67` (an auxiliary regression loss
directly supervising the readout toward `p66`'s own positive nonlinear
finding, NEUTRAL, flat across a 4x lambda sweep) all failed to. So H6 is
well-evidenced as a **description** and unproven as a **lever** — and a
model paper needs the lever, not the description. Every additive,
frozen-readout, and readout-training-signal intervention this programme has
designed has now been tried and failed; the one remaining untested class is
a **jointly retrained** encoder (not a post-hoc probe or an auxiliary loss
on a frozen trunk), which needs an actual GPU training pilot before H6 can
be called exhausted rather than merely well-evidenced. Its prior payoff is
now lower than it looked before `p65`/`p67`: the readout could not convert
even a directly, explicitly supervised, information-theoretically-real
signal into discrimination, which locates part of the bottleneck downstream
of "is the information retained," not only in "was it asked for."
