...
OUT_ROOT=${OUT_ROOT:-runs/pure_next}
...
case "${PURE_PHASE}" in
  ...
  scaling)
    ...
    DEFAULT_RELATION_CONTEXT_LAYERS=2
    DEFAULT_LOGIT_ADJ_TAU=0.5
    DEFAULT_FREQ_BIAS_ENABLED=false
    ;;
  eval)
    DEFAULT_OBJECT_LANGUAGE_ANCHOR_ENABLED=true
...
echo "[PURE-next] phase=${PURE_PHASE} save=${SAVE_PATH} resume=${RESUME_FROM:-none} lr=${LR} epochs=${EPOCHS} freeze_clip=${FREEZE_CLIP} anchor=${OBJECT_LANGUAGE_ANCHOR_ENABLED} rel_ctx=${RELATION_CONTEXT_LAYERS} logit_adj_tau=${LOGIT_ADJ_TAU} freq_alpha=${FREQ_BIAS_ALPHA}"
